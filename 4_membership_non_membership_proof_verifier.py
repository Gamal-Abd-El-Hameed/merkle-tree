...
def verify_membership(merkle_root, proof_hashes, pos, value):
    ...
    for proof_hash in proof_hashes:
        if pos % 2 == 0:
            root_hash = calculate_parent_hash(root_hash, proof_hash)
        else:
            root_hash = calculate_parent_hash(proof_hash, root_hash)
        pos = pos // 2
    ###### YOUR CODE ENDS HERE #############
...
def verify_non_membership(merkle_root, lower_bound_proof_hashes, upper_bound_proof_hashes,
    ...
    if lower_bound_pos != -1 and lower_bound_value >= target_value:
        return False
    if upper_bound_pos != -1 and upper_bound_value <= target_value:
        return False
    if lower_bound_pos != -1 and upper_bound_pos != -1 and upper_bound_pos - lower_bound_pos != 1:
        return False
    if lower_bound_pos != -1 and not verify_membership(merkle_root, lower_bound_proof_hashes, lower_bound_pos,
                                                       lower_bound_value):
        return False
    if upper_bound_pos != -1 and not verify_membership(merkle_root, upper_bound_proof_hashes, upper_bound_pos,
                                                       upper_bound_value):
        return False
    ######### YOUR CODE ENDS HERE #########
